...
const float TEMP_C_IF_INVALID = -100.00;
const float TEMP_F_IF_INVALID = -200.00;
...
//Take temp_sensor_value that is an int between 0 and 1023 and converts it to a float temp_celsius between -55 and 125
float temp_sensor_converter(int temp_sensor_value, bool *success)
{
  ...
  //y=mx+b, y is the temp_c, m is the TEMP_SLOPE, x is the temp_sense, and b is the TEMP_C_Min
  ...
  printf("Temperature sensor value (%d) was converted sucessfully to temperature %f C in the range of %f C to %f C.\n", temp_sensor_value, temp_celsius, TEMP_C_MIN, TEMP_C_MAX);
  *success = true;
  return temp_celsius;
}
